...
int main()
{
    ...
    do   //In ASCII, Ctrl+D is 4, and Ctrl+Z is 26
    {   if (isalnum(letter)) //Characters considered are only letters, not spaces, or anything else.
        num_char++;
        
        else if (letter == 10)   // 10 = Line feed, 13 = carriage return
        {
            num_lines++;
        ...
        prev_letter = letter;
    }while ((letter = getchar()) != 4 && letter!= 26 && letter != EOF);
