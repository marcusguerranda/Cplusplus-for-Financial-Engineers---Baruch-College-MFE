...
Array::Array()
{
    ...
    m_data  = new Point[m_size];                                                          //Allocating 10 elements to temporary array on the heap
    if(m_data == nullptr)
    {
...
Array::Array(const int &given_size): m_size(given_size), m_data(new Point[m_size])          //Constructor with size argument, taking const reference
{
    m_size = given_size;                                                                    
    m_data = new Point[m_size];                                                             //Creating an array on the heap with an input size argument
    if(m_data == nullptr)
    {
        std::cout << "User, no memory was available to temporarily store an array on the heap." << std::endl; //No memory available on theap
    }
...
{
    m_size = a.m_size;                                                                      //Copying the same size of the array provided into our array
    m_data = new Point[m_size];                                                             //Creating the array with the same size as the array provided
    if(m_data = nullptr)
    {
        std::cout << "User, no memory was available to temporarily store an array on the heap." << std::endl; //No memory available on theap
    }
...
Array& Array::operator = (const Array &a)                                                   //Assignment operator, as done previously for Point, Line, and Circle classes. 
{
    if(this == &a)                                                                          //Precluding self-asignment
    {       
        return *this;
    }
    else
    {
        ...
        m_size = a.m_size;                                                                  //Create a new array and copy each individual element with for-loop
...
const Point& Array::GetElement(int index) const                             //Getter function. When the index is out of bounds, return the first element
{
...
void Array::SetElement(const Point& p, int index)                         //Functions that sets an element. When the index is ount of bounds, ignore the "set"
{                                                                         //Using const reference for the point, and int value of the index for the array
    if(index < m_size && index >= 0)
    {
        m_data[index] = p;                                                //Set given Point's value(s) into m_data (pointer to a Point at index p_index in an array )
    }
    else
    {
